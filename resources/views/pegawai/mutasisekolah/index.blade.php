        ...
        <div class="col-12">
            <div class="card" style="background-color: white; border-radius: 15px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                <div class="card-header d-flex justify-content-between align-items-center" style="border-radius: 15px 15px 0 0;  background: linear-gradient(to right, #28A745, #DEE9DE, #28A745, #DEE9DE);">
                    <h3 class="card-title m-0" style="font-weight: bold; color: #FFFFFF;">Data Mutasi Sekolah</h3>

                    <a href="{{ url('pegawai/mutasisekolah/create') }}" class="btn btn-success ml-auto"> <span class="fa fa-plus"></span> </a>
                ...
                <div class="card-body">
                    <table id="example2" class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach ($list_mutasisekolah as $mutasisekolah)
                            <tr>
                                <td class="text-center">{{ $loop->iteration }}</td>
                                <td class="text-center">
                                    <div class="crud-buttons">
                                        <a href="{{ url("pegawai/mutasisekolah/$mutasisekolah->id") }}" class="btn btn-secondary" title="View">
                                            <span class="fa fa-eye"></span>
                                        </a>
                                        @if($mutasisekolah->status != 'Diterima' && $mutasisekolah->status != 'Ditolak')
                                        <a href="{{ url("pegawai/mutasisekolah/$mutasisekolah->id/edit") }}" class="btn btn-warning" title="Edit">
                                            <span class="fa fa-edit"></span>
                                        </a>
                                        <form action="{{ url('pegawai/mutasisekolah', $mutasisekolah->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <td class="text-justify">{{$mutasisekolah->nama}}</td>
                                <td class="text-justify">{{$mutasisekolah->nip}}</td>
                                <td class="text-center">
                                ...
                                <td class="text-justify">{{$mutasisekolah->pesan}}</td>
                                <!-- <td class="text-center" style="width: 25%">
                                    <img src="{{ url("public/app/mutasi_sekolah/foto/$mutasisekolah->foto") }}" class="img-responsive" style="width:50%">
                                </td> -->
                            </tr>
