        ...
        <div class="col-12">
            <div class="card" style="background-color: white; border-radius: 15px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                <div class="card-header d-flex justify-content-between align-items-center" style="border-radius: 15px 15px 0 0;  background: linear-gradient(to right, #28A745, #DEE9DE, #28A745, #DEE9DE);">
                    <h3 class="card-title m-0" style="font-weight: bold; color: #FFFFFF;">Data Pensiun Dini & Uzur</h3>

                    <a href="{{ url('pegawai/pensiundizur/create') }}" class="btn btn-success ml-auto"> <span class="fa fa-plus"></span></a>
                </div>
                ...
                <div class="card-body">
                    <table id="example2" class="table table-bordered table-hover">
                        ...
                        <tbody>
                            @foreach ($list_pensiundizur as $pensiundizur)
                            <tr>
                                <td class="text-center">{{ $loop->iteration }}</td>
                                <td class="text-center">
                                    <div class="crud-buttons">
                                        <a href="{{ url("pegawai/pensiundizur/$pensiundizur->id") }}" class="btn btn-secondary" title="View">
                                            <span class="fa fa-eye"></span>
                                        </a>
                                        @if($pensiundizur->status != 'Diterima' && $pensiundizur->status != 'Ditolak')
                                        <a href="{{ url("pegawai/pensiundizur/$pensiundizur->id/edit") }}" class="btn btn-warning" title="Edit">
                                            <span class="fa fa-edit"></span>
                                        </a>
                                        <form action="{{ url('pegawai/pensiundizur', $pensiundizur->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <td class="text-justify">{{$pensiundizur->nama}}</td>
                                <td class="text-justify">{{$pensiundizur->nip}}</td>
                                <td class="text-center">
                                ...
                                <!-- <td class="text-center" style="width: 25%">
                                    <img src="{{ url("public/app/pensiun_dizur/foto/$pensiundizur->foto") }}" class="img-responsive" style="width:50%">
                                </td> -->
                            </tr>
